...
public class TakeReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        Toast.makeText(context.getApplicationContext(), "Taken", Toast.LENGTH_SHORT).show();

        // Online / Offline user
        if (sharedPrefManager.getEmail().equals("OFFLINE-USER")) {
            HistoryDB historyDB = new HistoryDB(context);
            History history = new History(medName, medDate, medTime, "Taken");
            historyDB.addHistory(history);
        } else {
            takeOnline(context, medName, medDate, medTime);
        }
    }

    // Take online alarm ( Log History )
    public void takeOnline(Context context, String medName, String medDate, String medTime) {
        requestQueue = Volley.newRequestQueue(context);
        String url = "http://192.168.0.81/remindme/api/loghistory.php";
